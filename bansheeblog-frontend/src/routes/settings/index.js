...
export default class Settings extends Component {
	...
	openUpdateDialog = async () => {
    	if (this.state.updatesFound && this.state.updatesFound.Available) {
		...
		this.setState({ updatesFound: updates });
    ...
    deleteThemeOrFile = async () => {
    	...
    	const response = await Delete(`/api/${type}`, name, false);
    	if (response.ok) {
					...
					<Dialog.Body>
						{state.updatesFound && state.updatesFound.Available ? (
							<span>
								...
								<div>
									<Typography body1>Version: <b>{state.updatesFound.Version}</b></Typography>
								</div>
								<div>
									<Typography body2>Released <i>{new Date(state.updatesFound.Released).toLocaleString()}</i></Typography>
								</div>
								<div>
									<Typography body2>
										<a href={state.updatesFound.Url} target="_blank">
											<i>Release notes</i>
										</a>
									</Typography>
								</div>
								<Typography body2>
									Info:
									<Markup markup={state.updatesFound.Message}/>
								</Typography>
								<br/>
								<Typography body2>
									<i>
